...
def write_batches(target_dir, name, start_batch_num, labels, jpeg_files):
    ...
    for i,(labels_batch, jpeg_file_batch) in enumerate(zip(labels, jpeg_files)):
	    ...
	    print i, jpeg_file_batch[0].name
	...
	pickle('corrupted_list_should_not_be_here', corrupt_list)
    return i + 1
...
if __name__ == "__main__":
    ...
    labels_dic, label_names = generate_labels()
    ...
    with open_tar(TRAIN_TAR, 'training tar') as tf:
        ...
        train_labels = [[labels_dic[jpeg.name.split('/')[-1].split('_')[0]]] for jpeg in train_jpeg_files]
        print "done"
    ...
    #    validation_labels = [[labels_dic[jpeg.name.split('/')[-1].split('_')[0]]] for jpeg in validation_jpeg_files]
