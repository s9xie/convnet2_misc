...
class OCRDataProvider(LabeledDataProvider):
    def __init__(self, data_dir, batch_range=None, init_epoch=1, init_batchnum=None, dp_params=None, test=False):
        LabeledDataProvider.__init__(self, data_dir, batch_range, init_epoch, init_batchnum, dp_params, test)
        self.img_size = 32 
        self.num_colors = 1
        ...
        for i in batch_range:
            ...
            self.data_dic[-1]['context'] = n.require(self.data_dic[-1]['context'], dtype=n.single, requirements='C')
    ...
    def get_next_batch(self):
        ...
        self.batches_generated += 1
        return epoch, batchnum, [cropped, self.data_dic[bidx]['context']], self.data_dic[bidx]['labels']
...
class TasksCotrDataProvider(LabeledDataProvider):
    ...
    def get_data_dims(self, idx=0):
        ...
        if idx == 2:
            return 1
	    #return self.get_num_classes() Why????????
        return 1
    ...
    def get_next_batch(self):
        ...
        return epoch, batchnum, [self.data[self.d_idx]['data'].T, self.data[self.d_idx]['labvec'].T, self.data[self.d_idx]['taskvec'].T]
